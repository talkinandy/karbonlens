/**
 * POST /api/admin/match-queue/defer — T21 defer handler.
 *
 * Marks the queue row as `deferred` (v0.1 canonical status — migration 001
 * documents the domain but the column is plain TEXT with no CHECK, so no
 * DDL change is required). `resolved_by` and `resolved_at` remain NULL —
 * defer is NOT a final resolution.
 *
 * Body: `{ id: string }`.
 */
...
export async function POST(request: Request) {
  ...
  if (rows[0].status === 'deferred') {
    return NextResponse.json(
      { error: 'Row is already deferred' },
      { status: 409 },
    );
  }
  if (rows[0].status !== 'pending') {
    return NextResponse.json(
      { error: `Cannot defer — row is already ${rows[0].status}` },
      { status: 409 },
    );
  }
  ...
  try {
    ...
    console.error('[admin/match-queue/defer] failed', err);
    const msg = err instanceof Error ? err.message : String(err);
    return NextResponse.json(
      { error: `Defer failed: ${msg}` },
      { status: 500 },
    );
  }
